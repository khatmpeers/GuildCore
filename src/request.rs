pub mod request {
    ...
    impl RequestStub {
        pub fn new(
            ...
            let labels: HashMap<String, String> = if let Some(col) = labels { col } else { HashMap::new() };
        ...
        pub fn to_request<T: Rewardable + Serialize + DeserializeOwned>(self: Self, id: &str, reward: Option<T>) -> Request<T> {
            Request {
                id: id.to_string(),
                title: self.title,
        ...
        pub fn to_draft(self: Box<Self>, id: &str) -> RequestDraft {
            RequestDraft {
                id: id.to_string(),
                title: self.title,
                description: self.description,
    ...
    impl RequestDraft {
        ...
        pub async fn publish<T: Rewardable + Serialize + DeserializeOwned>(self: Self, reward: Option<T>, pool: &SqlitePool) -> anyhow::Result<Request<T>> {
            let request = Request {
                id: self.id,
                title: self.title,
                description: self.description,
    ...
    impl<T: Rewardable + Serialize + DeserializeOwned> Request<T> {
       ...
       pub async fn claim(self: Self, member_id: &str, pool: &SqlitePool) -> anyhow::Result<AcceptedRequest<T>> {
    ...
    impl<T: Rewardable + Serialize + DeserializeOwned> AcceptedRequest<T> {
        ...
        pub fn complete(self: Self) -> () {
            ...
            // maybe we create a CompletedRequest struct later, but for now this is good.
        }
    }
